...
const CorporateReview = ({ nextAction }: { nextAction: () => void }) => {
  ...
  return (
    <div className="h-full w-full space-y-12 p-8">
      ...
      {selectedPage === 0 && <CorporatePersonalInfo next={next} isReview />}
      {selectedPage === 1 && (
        <CorporateProofIdentity next={next} back={next} isReview />
      )}
      {selectedPage === 2 && <CorporateBusinessInfo next={next} isReview />}
    </div>
  );
};
export default memo(CorporateReview);
