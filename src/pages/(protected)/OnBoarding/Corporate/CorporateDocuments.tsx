...
import CorporateIncorpUpload from './CorporateIncorpUpload';
import CorporateBusAddUpload from './CorporateBusAddUpload';
import CorporateSourceOfFundsUpload from './CorporateSourceOfFundsUpload';
import CorporateShareholderCardUpload from './CorporateShareholderCardUpload';

type DocumentType = 'inc' | 'pos' | 'pob' | 'shareholder' | undefined;

const CorporateDocuments = ({ next }: { next: () => void }) => {
  ...
  if (documentType === 'inc') {
    return <CorporateIncorpUpload back={back} next={updateState} />;
  }

  if (documentType === 'pob') {
    return <CorporateBusAddUpload back={back} next={updateState} />;
  }

  if (documentType === 'pos') {
    return <CorporateSourceOfFundsUpload back={back} next={updateState} />;
  }

  if (documentType === 'shareholder') {
    return <CorporateShareholderCardUpload back={back} next={updateState} />;
  }
    ...
    <div className="h-full w-full space-y-8 p-8">
      ...
      <div className="grid grid-cols-2 gap-6 gap-x-4 max-lg:grid-cols-1">
        <Card
          size="small"
          role="button"
          onClick={() => setDocumentType('shareholder')}
            ...
            <Space>
              ...
              <div className="space-y-0.5">
                ...
                <p className="text-sm text-grey-500">
                  Upload shareholder ID Card
                </p>
              </div>
            </Space>
            {uploadState.shareholder && (
...
export default memo(CorporateDocuments);
